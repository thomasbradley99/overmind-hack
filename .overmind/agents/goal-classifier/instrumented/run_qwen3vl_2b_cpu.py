#!/usr/bin/env python3
"""Run qwen3-vl:2b on quarter dataset - try smaller images and CPU fallback."""
...
def classify(model: str, clip_path: str, prompt: str, timeout: int = 600, max_size: int = 112) -> dict:
    frame = extract_frame(clip_path, max_size=max_size)
...
if __name__ == "__main__":
    ...
    print(f"GPU: AMD HawkPoint1 (integrated)")
    print(f"Strategy: Using 112px images (smaller = faster)")
    ...
    for i, clip in enumerate(clips):
        ...
        try:
            result = classify(MODEL, str(clip), prompt, timeout=600, max_size=112)
