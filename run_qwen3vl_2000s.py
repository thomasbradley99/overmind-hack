#!/usr/bin/env python3
"""Run qwen3-vl:8b on quarter dataset with 2000s timeout."""
...
if __name__ == "__main__":
    ...
    print(f"Model: {MODEL}")
    print(f"Timeout: {TIMEOUT}s per clip")
    print(f"Clips: {len(clips)}")
    print(f"Estimated total time: ~{len(clips) * 5 / 60:.1f} hours (if 300s avg) or much longer")
    ...
    for i, clip in enumerate(clips):
        ...
        print(f"[{i+1}/{len(clips)}] {clip.name} (truth: {truth}, team: {truth_team}) — timeout={TIMEOUT}s")
        try:
            ...
            # Print incremental CM every clip
            print_cm(results, f"{MODEL} — INCREMENTAL after {i+1} clips")
