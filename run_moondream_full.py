...
# Use descriptive prompt that worked in the earlier test
PROMPT = "Describe this football image in detail. Is the ball fully inside the goal net?"
...
for i, clip in enumerate(clips, 1):
    ...
    # Parse: if raw mentions "goal" in a positive context, consider it a goal prediction
    raw_lower = raw.lower()
    if "goal" in raw_lower and "not" not in raw_lower.split("goal")[0].split()[-3:]:
        pred = "goal"
    else:
        pred = "not_goal"
...
print(f"\n{'='*70}")
print(f"moondream:1.8b — 224px SINGLE FRAME — FINAL")
print(f"{'='*70}")
