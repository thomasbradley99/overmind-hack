#!/usr/bin/env python3
"""Run both models on full 9-8GT-right dataset with 1 frame, 224px, 600s timeout."""
...
def extract_frame(clip_path: str, max_size: int = 224) -> str:
    work_dir = Path(tempfile.mkdtemp(prefix="full-"))
...
def classify(model: str, clip_path: str, prompt: str) -> dict:
...
if __name__ == "__main__":
    ...
    print(f"Frame: 1 frame, 224px, middle of clip")
    print(f"Timeout: 600s per clip")
    ...
    for model in MODELS:
        ...
        for i, clip in enumerate(clips):
            ...
            print(f"[{i+1}/{len(clips)}] Processing {clip.name}...", flush=True)
            try:
                t0 = time.time()
                ...
                # Print incremental CM every 5 clips
                if (i + 1) % 5 == 0 or i == len(clips) - 1:
                    print_cm(results, f"{model} — INCREMENTAL")
    ...
    print(f"{'#'*60}")
    os.system("python3 compare_models.py")
