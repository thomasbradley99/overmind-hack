#!/usr/bin/env python3
"""Run qwen3-vl:8b on quarter dataset with optimized settings."""
...
def extract_frame(clip_path: str, max_size: int = 112) -> str:
    work_dir = Path(tempfile.mkdtemp(prefix="qwen8-"))
...
def classify(model: str, clip_path: str, prompt: str, timeout: int = 3600) -> dict:
...
if __name__ == "__main__":
    ...
    print(f"Strategy: 112px images, reduced context (512), 1hr timeout per clip")
    ...
    for i, clip in enumerate(clips):
        ...
        try:
            result = classify(MODEL, str(clip), prompt, timeout=3600)
